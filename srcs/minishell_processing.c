...
int	new_token_size(t_var *var, char *token)
{
	...
	while (token[i])
	{
		if (token[i] == '\\' && is_escape_char(token[i + 1]))
			len--;
...
char	*token_builder(t_var *var, char *token)
{
	...
	while (i < len)
	{
		if (token[j] == '\\' && is_escape_char(token[j + 1]))
		{
			new_token[i] = token[j + 1];
			j++;
		}
		else if (token[j] == '$' && ft_isalnum(token[j + 1]))
		{
...
char	**clean_cmd(t_var *var, char **cmd)
{
	...
	while (cmd[i])
	{
		...
		if (!cmd[i])
			break;//NOTE: do something for malloc failure
		free(tmp);
		i++;
	}
	return (cmd);
...
_Bool	ms_cmd_cleaner(t_var *var)
{
	...
	while(var->cmds[i])
	{
		var->cmds[i] = clean_cmd(var, var->cmds[i]); //NOTE: do something for malloc failure
		i++;
	}
	return false;
}
